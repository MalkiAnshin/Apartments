...
const ProjectsImages: React.FC<ProjectImagesProps> = ({ property_id }) => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch('/api/projectsPictures', {
          method: 'GET',
          headers: {
    ...
    <div className="relative w-full h-64 md:h-96">
      {images.length > 0 ? (
        <img
          ...
          alt={`Project ${property_id} - Image ${currentIndex + 1}`}
