...
const ShowProjects: React.FC = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await fetch(`/api/AdminInterface/ShowAsset/ShowProjects?city=${encodedCity}`);
      if (!response.ok) {
  ...
  // פונקציה למחיקת נכס
  const deleteProjects = async (propertyId: string) => {
    if (!window.confirm('האם אתה בטוח שברצונך למחוק נכס זה?')) {
      return;
    }
    try {
      const response = await fetch(`/api/AdminInterface/DeletedAsset/DeleteProject?property_id=${propertyId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
      ...
      // ריענון הרשימה
      setProjects(projects.filter((project) => project.property_id !== propertyId));
    } catch (err) {
  ...
  const filteredProjects = projects.filter((project) => {
    const isInPriceRange = project.price >= priceFilter.min && project.price <= priceFilter.max;
    const isInRoomsRange = roomsFilter ? project.rooms === roomsFilter : true;
  ...
  return (
    <div className="bg-transparent text-white min-h-screen flex flex-col items-center p-6">
      <div className="w-full max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 text-gold text-center">רשימת נכסים</h1>
            ...
            <tbody>
              {filteredProjects.map((project) => (
                <tr key={project.property_id} className={`border-b border-gold ${selectedProperty === project.property_id ? 'bg-gold/10' : ''}`}>
                  ...
                  <td className="p-4">
                    <button
                      onClick={() => fetchContracts(project.property_id)}
