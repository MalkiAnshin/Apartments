...
const ShowBusiness: React.FC = () => {
  ...
  const fetchBusiness = async () => {
    try {
      ...
      const response = await fetch(`/api/AdminInterface/ShowAsset/ShowBusiness?city=${encodedCity}`);
      if (!response.ok) {
      ...
      setError('Error fetching business: ' + (err instanceof Error ? err.message : 'Unknown error'));
      console.error(err);
    }
  };

  // פונקציה למחיקת דירה
  const deleteBusiness = async (propertyId: string) => {
    if (!window.confirm('האם אתה בטוח שברצונך למחוק נכס זה?')) {
      return;
    }
    try {
      const response = await fetch(`/api/AdminInterface/DeletedAsset/DeleteBusiness?property_id=${propertyId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
      ...
      // ריענון הרשימה
      setBusiness(business.filter((business) => business.property_id !== propertyId));
    } catch (err) {
      setError('Error deleting business: ' + (err instanceof Error ? err.message : 'Unknown error'));
      console.error(err);
    }
  };
  ...
  const filteredBusiness = business.filter((business) => {
    const isInPriceRange = business.price >= priceFilter.min && business.price <= priceFilter.max;
  ...
  return (
    <div className="bg-transparent text-white min-h-screen flex flex-col items-center p-6">
      <div className="w-full max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 text-gold text-center">רשימת בתי עסק</h1>
          ...
          <table className="min-w-full bg-transparent border border-gold mb-6">
            ...
            <tbody>
              {filteredBusiness.map((business) => (
                <tr key={business.property_id} className={`border-b border-gold ${selectedProperty === business.property_id ? 'bg-gold/10' : ''}`}>
                  <td className="p-4">{business.property_id}</td>
                  <td className="p-4">{business.city}</td>
                  <td className="p-4">
                    {Number(business.price).toLocaleString('he-IL', { maximumFractionDigits: 0 })}
                  </td>
                  <td className="p-4">{business.rooms}</td>
                  <button
                    onClick={() => deleteBusiness(business.property_id)}
                  ...
                  <td className="p-4">
                    <button
                      onClick={() => fetchContracts(business.property_id)}
